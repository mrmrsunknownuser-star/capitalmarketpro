    ...
    <div className="w-full max-w-md">
      ...
      <div className="bg-[#0d1117] border border-[#21262d] rounded-2xl p-8">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-sm text-[#8b949e] mb-2">Full name</label>
            <input
              type="text"
              value={fullName}
          ...
          <div>
            <label className="block text-sm text-[#8b949e] mb-2">Confirm password</label>
            <input
              type="password"
          ...
          <div className="flex gap-1">
            {[1,2,3,4].map((i) => (
              <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${
                password.length === 0 ? 'bg-[#21262d]' :
                password.length < 6 && i === 1 ? 'bg-red-500' :
                password.length < 8 && i <= 2 ? 'bg-yellow-500' :
                password.length < 12 && i <= 3 ? 'bg-blue-500' :
                password.length >= 12 ? 'bg-green-500' : 'bg-[#21262d]'
              }`} />
            ))}
          </div>
