...
export default function AdminDepositsPage() {
  ...
  const fetchDeposits = async () => {
    ...
    if (error) {
      console.error('Deposits fetch error:', error.message)
      setLoading(false)
      return
    }
      ...
      deps.map(async dep => {
        try {
          const { data: user } = await supabase
            ...
            .eq('id', dep.user_id)
            .single()
          return { ...dep, user: user || null }
        } catch {
          return { ...dep, user: null }
        }
      })
    )

    setDeposits(withUsers)
    setLoading(false)
  }
  ...
  useEffect(() => {
    fetchDeposits()
    const supabase = createClient()
    supabase.channel('admin-deposits-live')
  ...
  const handleAction = async (status: 'approved' | 'rejected') => {
    ...
    if (status === 'approved') {
      ...
      await supabase.from('notifications').insert({
        ...
        message: `Your deposit of $${selected.amount?.toLocaleString()} via ${selected.crypto || 'BTC'} has been confirmed and credited! New balance: $${newBalance.toLocaleString()}.`,
        type: 'success',
        is_read: false,
        recipient_role: 'user',
      })
        ...
        if (selected.user?.email) {
          await fetch('/api/email', {
            ...
            body: JSON.stringify({
              ...
              data: { amount: selected.amount, crypto: selected.crypto, newBalance },
            }),
          })
        }
      } catch {}
      ...
      await supabase.from('notifications').insert({
        user_id: selected.user_id,
        title: '❌ Deposit Issue',
        message: `Your deposit of $${selected.amount?.toLocaleString()} could not be processed. ${note ? 'Reason: ' + note : 'Please contact support.'}`,
        type: 'warning',
        is_read: false,
        recipient_role: 'user',
      })
    ...
    try {
      await supabase.from('audit_logs').insert({
        ...
        details: { amount: selected.amount, crypto: selected.crypto, tx_hash: selected.tx_hash, note },
      })
    } catch {}

    setMessage(`✅ Deposit ${status}${status === 'approved' ? ` — $${selected.amount?.toLocaleString()} credited` : ''}`)
  ...
  const todayTotal = deposits.filter(d => new Date(d.created_at).toDateString() === new Date().toDateString()).reduce((s, d) => s + (d.amount || 0), 0)
    ...
    <div style={{ padding: 20, fontFamily: 'monospace' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
        {[
          ...
          { l: "Today's Volume", v: `$${todayTotal.toLocaleString()}`, c: '#C9A84C', icon: '📈' },
        ].map(s => (
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search user or TX hash..."
      ...
      <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} style={{ padding: 48, textAlign: 'center', color: '#484f58' }}>Loading deposits...</td></tr>
              ) : filtered.length === 0 ? (
                <tr>
                ...
                <tr key={dep.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #161b22' : 'none' }}>
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: '#484f58', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{dep.user?.email || dep.user_id?.slice(0, 16) + '...'}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                  ...
                  <td style={{ padding: '12px 14px', fontSize: 10, color: '#8b949e', fontFamily: 'monospace', maxWidth: 120 }}>
                    {dep.tx_hash ? dep.tx_hash.slice(0, 18) + '...' : '—'}
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: sc(dep.status), background: `${sc(dep.status)}15`, border: `1px solid ${sc(dep.status)}33`, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                      {si(dep.status)} {dep.status}
                    </span>
                  </td>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {[
                ...
                <div key={item.l} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #161b22', gap: 12 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#e6edf3', fontWeight: 500, textAlign: 'right', wordBreak: 'break-all', fontFamily: item.l === 'Transaction Hash' ? 'monospace' : 'inherit' }}>{item.v}</span>
                </div>
              ))}
              ...
              {selected.tx_hash && selected.tx_hash !== 'Not provided' && (
                <a href={`https://blockchair.com/bitcoin/transaction/${selected.tx_hash}`} target="_blank" rel="noopener noreferrer"
                  style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#C9A84C', textDecoration: 'none', marginTop: 14, padding: '10px 14px', background: 'rgba(201,168,76,0.06)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 10 }}>
                  <span>🔍</span><span>Verify on Blockchain Explorer →</span>
                </a>
              )}
                ...
                <>
                  ...
                  <div style={{ background: 'rgba(63,185,80,0.06)', border: '1px solid rgba(63,185,80,0.2)', borderRadius: 10, padding: '12px 14px', marginBottom: 18 }}>
                    <div style={{ fontSize: 11, color: '#3fb950', lineHeight: 1.8 }}>
                      ✅ Approving will automatically credit <strong>${selected.amount?.toLocaleString()}</strong> to the user's balance in real-time.
                    </div>
                  </div>
              ...
              {selected.status !== 'pending' && (
                <div style={{ marginTop: 16, background: `${sc(selected.status)}0d`, border: `1px solid ${sc(selected.status)}33`, borderRadius: 12, padding: 18, textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, color: sc(selected.status) }}>
                    {selected.status === 'approved' ? `Deposit approved — $${selected.amount?.toLocaleString()} credited` : 'Deposit rejected'}
                  </div>
