  ...
  const handleAction = async (status: 'approved' | 'rejected') => {
    ...
    await supabase.from('notifications').insert({
      ...
      message: status === 'approved'
        ? `Your withdrawal of $${selected.amount?.toLocaleString()} has been approved and is being processed. Funds will arrive within 24 hours.`
        : `Your withdrawal of $${selected.amount?.toLocaleString()} was not processed. ${note ? 'Reason: ' + note : 'Please contact support.'} ${status === 'rejected' ? 'Your balance has been refunded.' : ''}`,
    ...
    if (selected.user?.email) {
      await sendEmail(selected.user.email, status === 'approved' ? 'withdrawal_approved' : 'withdrawal_rejected', {
    ...
    setMessage(`✅ Withdrawal ${status}${status === 'rejected' ? ' — balance refunded to user' : ' — processing payment'}`)
      ...
      <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 640 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} style={{ padding: 48, textAlign: 'center', color: '#484f58' }}>Loading...</td></tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={8} style={{ padding: 48, textAlign: 'center' }}>
                    <div style={{ fontSize: 32, marginBottom: 10 }}>⬆</div>
                    <div style={{ fontSize: 13, color: '#484f58' }}>No {filter === 'all' ? '' : filter} withdrawals</div>
                  </td>
                </tr>
              ) : filtered.map((wd, i) => (
                <tr key={wd.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #161b22' : 'none' }}>
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#C9A84C,#E8D08C)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, color: '#060a0f', flexShrink: 0, overflow: 'hidden' }}>
                        {wd.user?.avatar_url ? <img src={wd.user.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (wd.user?.full_name || wd.user?.email || 'U')[0].toUpperCase()}
                      </div>
                      <div>
                        <div style={{ fontSize: 12, color: '#e6edf3', fontWeight: 500 }}>{wd.user?.full_name || 'Unknown'}</div>
                        <div style={{ fontSize: 10, color: '#484f58', maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{wd.user?.email}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <span style={{ fontSize: 11, color: '#F7A600', background: 'rgba(247,166,0,0.1)', padding: '3px 8px', borderRadius: 6, fontWeight: 700 }}>
                      {wd.network || 'BTC'}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '12px 14px', fontSize: 12, color: '#484f58' }}>
                    ${wd.fee?.toLocaleString() || (wd.amount * 0.05).toFixed(2)}
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: statusColor(wd.status), background: `${statusColor(wd.status)}15`, border: `1px solid ${statusColor(wd.status)}33`, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                      {statusIcon(wd.status)} {wd.status}
                    </span>
                  </td>
          ...
          <div style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: 20, width: '100%', maxWidth: 480, fontFamily: 'monospace', maxHeight: '92vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {[
                ...
                { l: 'Processing Fee', v: `$${selected.fee?.toFixed(2) || (selected.amount * 0.05).toFixed(2)} (5%)` },
              ...
              {selected.status === 'pending' && (
                <>
                  ...
                  <div style={{ background: 'rgba(248,81,73,0.06)', border: '1px solid rgba(248,81,73,0.2)', borderRadius: 10, padding: '12px 14px', marginBottom: 16 }}>
                    <div style={{ fontSize: 11, color: '#f85149', lineHeight: 1.8 }}>
                      ⚠ Send <strong>${(selected.amount - (selected.fee || selected.amount * 0.05)).toFixed(2)}</strong> to wallet: <strong style={{ fontFamily: 'monospace', wordBreak: 'break-all' }}>{selected.wallet_address}</strong>
                    </div>
                  </div>
                  ...
                  <div style={{ background: 'rgba(247,166,0,0.06)', border: '1px solid rgba(247,166,0,0.2)', borderRadius: 10, padding: '12px 14px', marginBottom: 18 }}>
                    <div style={{ fontSize: 11, color: '#F7A600', lineHeight: 1.8 }}>
                      📋 Rejecting will automatically refund <strong>${selected.amount?.toLocaleString()}</strong> back to the user's balance.
                    </div>
                  </div>
              ...
              {selected.status !== 'pending' && (
                <div style={{ marginTop: 16, background: `${statusColor(selected.status)}0d`, border: `1px solid ${statusColor(selected.status)}33`, borderRadius: 12, padding: 18, textAlign: 'center' }}>
                  <div style={{ fontSize: 32, marginBottom: 8 }}>{statusIcon(selected.status)}</div>
