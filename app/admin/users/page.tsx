...
export default function AdminUsersPage() {
  ...
  const update = async (userId: string, fields: any, log: string) => {
    ...
    try { await supabase.from('audit_logs').insert({ action: log, target_user_id: userId, details: fields }) } catch {}
  ...
  const notify = async (userId: string, title: string, msg: string) => {
    const supabase = createClient()
    await supabase.from('notifications').insert({ user_id: userId, title, message: msg, type: 'info', is_read: false })
  }
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
            <thead>
              <tr style={{ background: '#0a0e14', borderBottom: '1px solid #161b22' }}>
                {['User', 'Status', 'KYC', 'Tier', 'BTC Address', 'Joined', ''].map(h => (
                  ...
                  <td style={{ padding: '12px 14px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'linear-gradient(135deg,#C9A84C,#E8D08C)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#060a0f', flexShrink: 0, overflow: 'hidden' }}>
                        {user.avatar_url ? <img src={user.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (user.full_name || user.email || 'U')[0].toUpperCase()}
                      </div>
                      <div>
                        <div style={{ fontSize: 13, color: '#e6edf3', fontWeight: 500 }}>{user.full_name || 'No name'}</div>
                        <div style={{ fontSize: 11, color: '#484f58', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.email}</div>
                      </div>
                    </div>
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: sc(user.status), background: `${sc(user.status)}18`, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase' }}>{user.status}</span>
                  </td>
                  <td style={{ padding: '12px 14px' }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: kc(user.kyc_status), background: `${kc(user.kyc_status)}18`, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase' }}>{user.kyc_status}</span>
                  </td>
                  <td style={{ padding: '12px 14px', fontSize: 11, color: '#8b949e', textTransform: 'capitalize' }}>{user.tier}</td>
                  <td style={{ padding: '12px 14px', fontSize: 10, color: user.btc_address ? '#3fb950' : '#484f58', fontFamily: 'monospace' }}>
                    {user.btc_address ? user.btc_address.slice(0, 12) + '...' : 'Not set'}
                  </td>
                  <td style={{ padding: '12px 14px', fontSize: 11, color: '#484f58' }}>{new Date(user.created_at).toLocaleDateString()}</td>
                  <td style={{ padding: '12px 14px' }}>
                    <button onClick={() => { setSelected(user); setBtcAddress(user.btc_address || '') }}
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              <div style={{ background: '#161b22', borderRadius: 12, padding: 16, marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                  <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg,#C9A84C,#E8D08C)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, fontWeight: 800, color: '#060a0f', overflow: 'hidden' }}>
                    {selected.avatar_url ? <img src={selected.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (selected.full_name || selected.email || 'U')[0].toUpperCase()}
                  </div>
                  <div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <input value={btcAddress} onChange={e => setBtcAddress(e.target.value)} placeholder="Enter new BTC address..."
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                  {[{ l: '✅ Active', s: 'active', c: '#3fb950' }, { l: '⏳ Pending', s: 'pending', c: '#F7A600' }, { l: '🔴 Suspend', s: 'suspended', c: '#f85149' }].map(a => (
                    <button key={a.s} onClick={() => update(selected.id, { status: a.s }, `USER_${a.s.toUpperCase()}`)} disabled={actionLoading || selected.status === a.s}
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                  {[{ t: 'starter', c: '#484f58' }, { t: 'silver', c: '#8b949e' }, { t: 'gold', c: '#C9A84C' }, { t: 'platinum', c: '#0052FF' }, { t: 'elite', c: '#7B2BF9' }, { t: 'black', c: '#e6edf3' }].map(item => (
                    <button key={item.t} onClick={() => update(selected.id, { tier: item.t }, 'TIER_UPDATED')} disabled={actionLoading}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                ...
                <button onClick={() => { notify(selected.id, '📣 Message from Admin', 'Our team has reviewed your account. Please contact support for details.'); setMessage('✅ Notification sent') }}
