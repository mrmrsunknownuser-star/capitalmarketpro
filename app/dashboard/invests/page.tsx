...
var PROGRAMS = [
  { id: 'trading', icon: '📡', label: 'Live Trading', color: '#2ecc71', bg: 'rgba(46,204,113,.12)', desc: 'Trade live with real-time charts and BUY/SELL', tag: 'LIVE', href: '/dashboard/trading' },
  { id: 'copy', icon: '📋', label: 'Copy Trading', color: '#C9A84C', bg: 'rgba(201,168,76,.12)', desc: 'Mirror top traders automatically and earn passively', tag: 'PASSIVE', href: '/dashboard/copy' },
  { id: 'affiliate', icon: '🤝', label: 'Affiliate Program', color: '#2ecc71', bg: 'rgba(46,204,113,.12)', desc: 'Earn 15% commission on every referral you make', tag: 'EARN FREE', href: '/dashboard/affiliate' },
  { id: 'signal', icon: '🔔', label: 'Trading Signals', color: '#3498db', bg: 'rgba(52,152,219,.12)', desc: 'AI-powered real-time buy and sell signals', tag: 'AI POWERED', href: '/dashboard/signal' },
  { id: 'portfolio', icon: '📊', label: 'My Portfolio', color: '#9b59b6', bg: 'rgba(155,89,182,.12)', desc: 'Track all your active investments with live ROI', tag: 'LIVE ROI', href: '/dashboard/portfolio' },
]
  ...
  return (
    <div style={{ background: '#060a0e', minHeight: '100vh', paddingBottom: 30, fontFamily: 'Inter, sans-serif' }}>
          ...
          {ALL_TYPES.map(function(type) {
            return (
              <Link key={type.id} href={'/dashboard/invest/' + type.id} style={{ textDecoration: 'none' }}>
                <div style={{ background: '#0d1117', border: '1px solid #1e2530', borderRadius: 18, padding: 18, cursor: 'pointer', position: 'relative', overflow: 'hidden' }}
                ...
                >
                  ...
                  <div style={{ width: 44, height: 44, borderRadius: 13, background: type.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: 12 }}>{type.icon}</div>
          ...
          {PROGRAMS.map(function(prog) {
            return (
              <Link key={prog.id} href={prog.href} style={{ textDecoration: 'none' }}>
                <div style={{ background: '#0d1117', border: '1px solid #1e2530', borderRadius: 18, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, cursor: 'pointer' }}
                  onMouseEnter={function(e) { e.currentTarget.style.borderColor = prog.color + '40' }}
                  onMouseLeave={function(e) { e.currentTarget.style.borderColor = '#1e2530' }}
                >
                  <div style={{ width: 50, height: 50, borderRadius: 16, background: prog.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>{prog.icon}</div>
                  <div style={{ flex: 1 }}>
      ...
      <div style={{ margin: '0 16px', background: 'linear-gradient(135deg,rgba(201,168,76,.08),rgba(201,168,76,.02))', border: '1px solid rgba(201,168,76,.2)', borderRadius: 18, padding: '20px' }}>
        ...
        <div style={{ fontSize: 13, color: '#8892a0', lineHeight: 1.7, marginBottom: 14 }}>Josh our account manager is available 24/7 to help you pick the right plan.</div>
        <Link href="/dashboard/support">
          <button style={{ width: '100%', padding: '13px', background: GG, border: 'none', borderRadius: 12, color: '#060a0e', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>Chat with Josh →</button>
        </Link>
      </div>
    </div>
  )
