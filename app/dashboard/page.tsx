...
const ASSETS = [
  ...
  { name: 'Ripple', symbol: 'XRP', icon: '✕', color: '#346AA9', price: 0.624, change: 4.1 },
  { name: 'Cardano', symbol: 'ADA', icon: '◆', color: '#0033AD', price: 0.482, change: 2.8 },
]
      ...
      <div style={{ background: '#0d1117', border: '1px solid #21262d', borderRadius: 20, width: '100%', maxWidth: 440, padding: 24, fontFamily: 'monospace', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#484f58' }}>Min. {modal.amount}</div>
          </div>
        ...
        {step === 1 && (
          <>
            ...
            <button onClick={() => setStep(2)} style={{ width: '100%', marginTop: 16, padding: '13px 0', background: `linear-gradient(135deg, ${modal.color}, ${modal.color}cc)`, border: 'none', borderRadius: 12, color: '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'monospace' }}>
  ...
  return (
    <div style={{ padding: '16px 16px 80px', fontFamily: 'monospace' }}>
      <style>{`
        ...
        .quick-actions { display: flex; gap: 16px; justify-content: space-around; }
      ...
      <div style={{ background: 'linear-gradient(135deg, #0d1117, #161b22)', borderRadius: 20, padding: '24px 20px', marginBottom: 20, border: '1px solid rgba(201,168,76,0.2)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div style={{ fontSize: 12, color: '#8b949e' }}>Welcome, <span style={{ color: '#C9A84C', fontWeight: 700 }}>{profileName}</span> 👋</div>
        ...
        <div style={{ marginBottom: 6 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 13, color: totalPnl >= 0 ? '#3fb950' : '#f85149', fontWeight: 600 }}>
              {totalPnl >= 0 ? '▲' : '▼'} {hideBalance ? '••••' : `$${Math.abs(totalPnl).toFixed(2)}`}
            </span>
            <span style={{ fontSize: 11, color: '#484f58' }}>all time</span>
          </div>
        </div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 14, background: '#0d1117', border: '1px solid #161b22', borderRadius: 10, padding: 3 }}>
          {['Crypto', 'Wallets'].map((tab, i) => (
            <button key={tab} onClick={() => setActiveAsset(i)} style={{ flex: 1, padding: '8px 0', borderRadius: 7, border: 'none', background: activeAsset === i ? '#161b22' : 'transparent', color: activeAsset === i ? '#C9A84C' : '#8b949e', fontSize: 12, cursor: 'pointer', fontFamily: 'monospace', fontWeight: activeAsset === i ? 700 : 400 }}>
              {tab}
            </button>
          ))}
        </div>
            ...
            {liveAssets.map((asset, i) => (
              <Link key={asset.symbol} href="/dashboard/market" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', padding: '14px 16px', borderBottom: i < liveAssets.length - 1 ? '1px solid #161b22' : 'none' }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: `${asset.color}18`, border: `1px solid ${asset.color}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: asset.color, marginRight: 14, flexShrink: 0 }}>
                    {asset.icon}
                  </div>
                  <div style={{ flex: 1 }}>
        ...
        {activeAsset === 1 && (
          <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, padding: 24, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 12, color: '#8b949e', marginBottom: 16 }}>Connect your external wallet to track all your assets</div>
            <Link href="/dashboard/deposit">
              <button style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #C9A84C, #E8D08C)', color: '#060a0f', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace' }}>Deposit Funds →</button>
            </Link>
          </div>
        )}
      </div>
      ...
      {/* ── AI BANNER ── */}
      <div style={{ background: 'linear-gradient(135deg, rgba(201,168,76,0.1), rgba(201,168,76,0.04))', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 14, padding: '18px 20px', marginBottom: 24, display: 'flex', gap: 14, alignItems: 'center' }}>
        <div style={{ fontSize: 32 }}>🤖</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#C9A84C', marginBottom: 4 }}>AI Trading — Active 24/7</div>
          <div style={{ fontSize: 11, color: '#8b949e', lineHeight: 1.6 }}>Our automated system is trading your funds right now. Select a plan to activate daily returns.</div>
        </div>
        <Link href="/dashboard/invest">
          <button style={{ padding: '9px 16px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #C9A84C, #E8D08C)', color: '#060a0f', fontSize: 11, fontWeight: 800, cursor: 'pointer', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>Invest →</button>
        </Link>
      </div>
        ...
        <div className="plans-grid">
          {INVESTMENT_PLANS.map(plan => (
            <div key={plan.name} onClick={() => setModal({ title: `${plan.name} Plan`, amount: plan.min, color: plan.color })}
              ...
              <div style={{ fontSize: 10, color: '#484f58', textAlign: 'center', marginBottom: 10 }}>Min: <span style={{ color: '#e6edf3', fontWeight: 600 }}>{plan.min}</span></div>
        ...
        <div className="signal-grid">
          {SIGNAL_PLANS.map(plan => (
            <div key={plan.name} onClick={() => setModal({ title: `${plan.name} Signal Plan`, amount: plan.price + '/mo', color: plan.color })}
              ...
              <button style={{ width: '100%', marginTop: 10, padding: '8px 0', borderRadius: 8, border: `1px solid ${plan.color}`, background: (plan as any).popular ? plan.color : `${plan.color}0d`, color: (plan as any).popular ? '#fff' : plan.color, fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'monospace' }}>
                Get →
              </button>
            </div>
          ))}
        ...
        <div style={{ background: 'linear-gradient(135deg, #0d1117, #161b22)', border: '1px solid rgba(201,168,76,0.25)', borderRadius: 16, padding: 20, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: '#C9A84C', marginBottom: 10, fontWeight: 600 }}>Account Manager · Senior Portfolio Manager</div>
              <div style={{ fontSize: 12, color: '#8b949e', lineHeight: 1.8, marginBottom: 14 }}>
                Former Goldman Sachs VP with <strong style={{ color: '#e6edf3' }}>14+ years</strong> managing over <strong style={{ color: '#C9A84C' }}>$2.4B</strong> in assets. Your dedicated guide to maximizing returns on CapitalMarket Pro.
              </div>
