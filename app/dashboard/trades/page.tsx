...
export default function TradesPage() {
  ...
  const [tab, setTab] = useState<'overview' | 'trades' | 'deposits' | 'withdrawals' | 'plans'>('overview')
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Realtime balance
      supabase
        .channel(`trades-balance-${user.id}`)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'balances', filter: `user_id=eq.${user.id}` }, payload => {
  ...
  const statusColor = (s: string) => ({ pending: '#F7A600', approved: '#3fb950', rejected: '#f85149', active: '#3fb950', completed: '#0052FF' }[s] || '#484f58')
  const statusIcon = (s: string) => ({ pending: '⏳', approved: '✅', rejected: '❌', active: '🟢', completed: '🏁' }[s] || '❓')
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ...trades.map(t => ['Trade', `${t.direction || 'BUY'} ${t.asset || 'BTC/USD'}`, t.amount, t.profit >= 0 ? 'Profit' : 'Loss', new Date(t.created_at).toLocaleDateString()]),
    ...
    a.href = url
    a.download = `capitalmarket-pro-statement-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
    ...
    return (
      <div style={{ padding: '16px 16px 80px', fontFamily: 'monospace' }}>
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ height: 14, background: '#161b22', borderRadius: 6, width: '25%', animation: 'pulse 1.5s ease infinite' }} />
        </div>
        {[1, 2, 3].map(i => (
    ...
    <div style={{ padding: '16px 16px 80px', fontFamily: 'monospace' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: '#484f58' }}>Track your investments and history</div>
        </div>
        <button onClick={exportCSV}
          style={{ padding: '9px 16px', borderRadius: 10, border: '1px solid #21262d', background: 'transparent', color: '#8b949e', fontSize: 11, cursor: 'pointer', fontFamily: 'monospace', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
          📥 Export CSV
        </button>
      </div>
      ...
      {activePlanCount > 0 && (
        <div style={{ background: 'rgba(63,185,80,0.06)', border: '1px solid rgba(63,185,80,0.2)', borderRadius: 14, padding: '16px 20px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#3fb950', boxShadow: '0 0 10px #3fb950' }} />
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#e6edf3', marginBottom: 2 }}>{activePlanCount} Active Investment Plan{activePlanCount > 1 ? 's' : ''}</div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: '#3fb950' }}>+${dailyEarnings.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10, marginBottom: 24 }}>
        <Link href="/dashboard/deposit" style={{ textDecoration: 'none' }}>
          <div style={{ background: 'rgba(63,185,80,0.08)', border: '1px solid rgba(63,185,80,0.2)', borderRadius: 12, padding: '14px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 20 }}>⬇</span>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#3fb950' }}>Deposit</div>
              <div style={{ fontSize: 10, color: '#484f58' }}>Add funds</div>
            </div>
          </div>
        </Link>
        <Link href="/dashboard/withdraw" style={{ textDecoration: 'none' }}>
          <div style={{ background: 'rgba(248,81,73,0.08)', border: '1px solid rgba(248,81,73,0.2)', borderRadius: 12, padding: '14px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 20 }}>⬆</span>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#f85149' }}>Withdraw</div>
              <div style={{ fontSize: 10, color: '#484f58' }}>Cash out</div>
            </div>
          </div>
        </Link>
        <Link href="/dashboard/invest" style={{ textDecoration: 'none' }}>
          <div style={{ background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 12, padding: '14px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 20 }}>💹</span>
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#C9A84C' }}>Invest</div>
              <div style={{ fontSize: 10, color: '#484f58' }}>Earn daily</div>
            </div>
          </div>
        </Link>
      </div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, marginBottom: 20 }}>
            {[
              ...
              { icon: '⬇', label: 'Total Deposited', value: `$${totalDeposited.toLocaleString()}`, color: '#C9A84C' },
              { icon: '⬆', label: 'Total Withdrawn', value: `$${totalWithdrawn.toLocaleString()}`, color: '#f85149' },
            ].map(item => (
          ...
          <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', borderBottom: '1px solid #161b22', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#e6edf3' }}>Recent Activity</div>
            </div>

            {[
              ...deposits.slice(0, 3).map(d => ({ ...d, kind: 'deposit', icon: '⬇', color: '#3fb950' })),
              ...withdrawals.slice(0, 3).map(w => ({ ...w, kind: 'withdrawal', icon: '⬆', color: '#f85149' })),
              ...activePlans.slice(0, 2).map(p => ({ ...p, kind: 'plan', icon: '💹', color: '#C9A84C', amount: p.amount })),
            ]
              .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
              .slice(0, 8)
              .map((item, i, arr) => (
                <div key={item.id || i} style={{ display: 'flex', alignItems: 'center', padding: '13px 16px', borderBottom: i < arr.length - 1 ? '1px solid #161b22' : 'none', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: `${item.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
                    {item.icon}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#e6edf3', marginBottom: 2, textTransform: 'capitalize' }}>
                      {item.kind === 'plan' ? `${(item as any).plan_name} Plan` : item.kind}
                    </div>
                    <div style={{ fontSize: 10, color: '#484f58' }}>
                      {item.kind === 'deposit' ? (item as any).crypto || 'BTC' : item.kind === 'withdrawal' ? (item as any).network || 'BTC' : `${(item as any).roi_percent}%/day`}
                      {' · '}
                      {new Date(item.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                    </div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: item.kind === 'withdrawal' ? '#f85149' : item.color }}>
                      {item.kind === 'withdrawal' ? '-' : '+'}${(item.amount || 0).toLocaleString()}
                    </div>
                    {item.kind !== 'plan' && (
            ...
            {deposits.length === 0 && withdrawals.length === 0 && activePlans.length === 0 && (
          ...
          {activePlans.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {activePlans.map(plan => {
                ...
                const planColors: Record<string, string> = { Starter: '#8b949e', Silver: '#8b949e', Gold: '#C9A84C', Platinum: '#0052FF', Elite: '#7B2BF9', Black: '#e6edf3' }
                  ...
                  <div key={plan.id} style={{ background: '#0d1117', border: `1px solid ${color}33`, borderRadius: 16, padding: 22 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: plan.status === 'active' ? '#3fb950' : '#0052FF', background: plan.status === 'active' ? 'rgba(63,185,80,0.1)' : 'rgba(0,82,255,0.1)', padding: '5px 12px', borderRadius: 20 }}>
                        {statusIcon(plan.status)} {plan.status === 'active' ? 'ACTIVE' : 'COMPLETED'}
                      </span>
                    </div>
          ...
          {trades.length === 0 ? (
            ...
            <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, overflow: 'hidden' }}>
              <div style={{ padding: '14px 16px', borderBottom: '1px solid #161b22', display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div style={{ fontSize: 12, color: totalPnl >= 0 ? '#3fb950' : '#f85149', fontWeight: 700 }}>
                  Total P&L: {totalPnl >= 0 ? '+' : ''}${totalPnl.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                </div>
              </div>
              {trades.map((t, i) => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', padding: '13px 16px', borderBottom: i < trades.length - 1 ? '1px solid #161b22' : 'none', gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#484f58' }}>{t.direction || 'BUY'} · {new Date(t.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: (t.profit || 0) >= 0 ? '#3fb950' : '#f85149', marginBottom: 2 }}>
            ...
            <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, overflow: 'hidden' }}>
              ...
              {deposits.map((d, i) => (
                <div key={d.id} style={{ display: 'flex', alignItems: 'center', padding: '13px 16px', borderBottom: i < deposits.length - 1 ? '1px solid #161b22' : 'none', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(201,168,76,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
                    ₿
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ background: '#0d1117', border: '1px solid #161b22', borderRadius: 14, overflow: 'hidden' }}>
              ...
              {withdrawals.map((w, i) => (
                <div key={w.id} style={{ display: 'flex', alignItems: 'center', padding: '13px 16px', borderBottom: i < withdrawals.length - 1 ? '1px solid #161b22' : 'none', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(248,81,73,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>
                    ⬆
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
