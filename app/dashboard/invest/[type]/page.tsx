...
var INVEST_DATA = {
  ...
  stocks: {
    ...
    desc: 'Invest in Tesla, Apple, NVIDIA, Amazon and 200+ global equities. Our professional stock desk manages your portfolio with proven strategies backed by fundamental and technical analysis.',
  ...
  forex: {
    ...
    desc: 'Trade EUR/USD, GBP/USD, USD/JPY and 70+ currency pairs. Our expert forex desk uses AI-powered signals and technical analysis to maximize your currency trading returns with deep liquidity.',
  ...
  realestate: {
    ...
    desc: 'Invest in premium real estate portfolios without the complexity of property ownership. Our fund provides stable, inflation-protected monthly returns from commercial and residential properties.',
  ...
  commodities: {
    ...
    desc: 'Invest in Gold, Silver, Crude Oil, Natural Gas and agricultural commodities. Commodities are historically the best hedge against inflation offering stable returns across all economic cycles.',
  ...
  indices: {
    ...
    desc: 'Trade the world\'s most recognized indices including S&P 500, NASDAQ 100, FTSE 100, DAX 40 and Nikkei 225. Index investing gives you diversified exposure to entire economies.',
  ...
  etf: {
    ...
    desc: 'Exchange Traded Funds offer the ultimate in diversification. Our curated ETF portfolios cover technology, healthcare, energy and emerging markets. Perfect for broad market exposure.',
  ...
  defi: {
    ...
    desc: 'Decentralized Finance represents the future of money. Our DeFi strategies include yield farming, liquidity provision, staking and automated market making across the top protocols.',
...
var LIVE_PRICES = {
  'BTC/USD': { price: '94,820', chg: '+4.07%', up: true },
  'ETH/USD': { price: '3,240', chg: '+1.87%', up: true },
  ...
  'GOLD': { price: '2,318', chg: '+0.64%', up: true },
  ...
  'SPX500': { price: '5,234', chg: '+0.92%', up: true },
  'NASDAQ100': { price: '18,420', chg: '+1.14%', up: true },
  'FTSE100': { price: '7,842', chg: '+0.34%', up: true },
  'DAX40': { price: '17,890', chg: '+0.52%', up: true },
  'NIKKEI225': { price: '38,240', chg: '+0.88%', up: true },
  'DOW JONES': { price: '38,940', chg: '+0.44%', up: true },
  ...
  'MKR/USD': { price: '1,840', chg: '+4.12%', up: true },
  'SNX/USD': { price: '2.84', chg: '+1.92%', up: true },
  'YFI/USD': { price: '6,240', chg: '+2.44%', up: true },
...
export default function InvestDetailPage() {
  ...
  if (!data) {
    return (
      <div style={{ minHeight: '100vh', background: '#060a0e', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: 'Inter, sans-serif' }}>
    ...
    <div style={{ background: '#060a0e', minHeight: '100vh', paddingBottom: 30, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ margin: '10px 16px 0', display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
        {['🔒 Funds Protected', '⚡ Instant Withdrawal', '📊 Daily Reports', '👤 Account Manager'].map(function(badge) {
          return (
            <div key={badge} style={{ flexShrink: 0, background: '#0d1117', border: '1px solid #1e2530', borderRadius: 100, padding: '6px 12px', fontSize: 11, color: '#8892a0', whiteSpace: 'nowrap' }}>{badge}</div>
          )
        })}
      </div>
        ...
        <div style={{ padding: '14px 16px 0' }}>
          <div style={{ background: '#0d1117', border: '1px solid #1e2530', borderRadius: 20, padding: '24px 20px', marginBottom: 14 }}>
            ...
            <Link href={'/dashboard/deposit?plan=' + encodeURIComponent(selectedPlan.name)}>
              <button style={{ width: '100%', padding: '15px', background: roiInputAmount > 0 ? GG : '#141920', border: 'none', borderRadius: 12, color: roiInputAmount > 0 ? '#060a0e' : '#4a5568', fontSize: 15, fontWeight: 800, cursor: roiInputAmount > 0 ? 'pointer' : 'not-allowed', fontFamily: 'Inter, sans-serif' }}>
          ...
          <div style={{ background: '#0d1117', border: '1px solid #1e2530', borderRadius: 14, padding: '14px 16px' }}>
            <div style={{ fontSize: 11, color: '#2a3140', lineHeight: 1.7 }}>⚠ Disclaimer: ROI calculations are estimates. Actual returns may vary. All investments carry risk.</div>
          </div>
        </div>
      )}
          ...
          {placed ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', background: '#0d1117', border: '1px solid #1e2530', borderRadius: 14, overflow: 'hidden' }}>
                <button onClick={function() { setTradeType('buy') }} style={{ flex: 1, padding: '15px', border: 'none', background: tradeType === 'buy' ? 'rgba(46,204,113,.15)' : 'transparent', color: tradeType === 'buy' ? '#2ecc71' : '#4a5568', fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'Inter, sans-serif', borderRight: '1px solid #1e2530' }}>
                  📈 BUY / LONG
                </button>
                <button onClick={function() { setTradeType('sell') }} style={{ flex: 1, padding: '15px', border: 'none', background: tradeType === 'sell' ? 'rgba(231,76,60,.15)' : 'transparent', color: tradeType === 'sell' ? '#e74c3c' : '#4a5568', fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
                  📉 SELL / SHORT
                </button>
              </div>
              ...
              <div style={{ background: '#0d1117', border: '1px solid #1e2530', borderRadius: 18, padding: '20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <button onClick={placeTrade} disabled={!tradeAmount || parseFloat(tradeAmount) <= 0 || placing} style={{ width: '100%', padding: '16px', background: !tradeAmount || parseFloat(tradeAmount) <= 0 ? '#141920' : tradeType === 'buy' ? 'linear-gradient(135deg,#27ae60,#2ecc71)' : 'linear-gradient(135deg,#c0392b,#e74c3c)', border: 'none', borderRadius: 14, color: !tradeAmount || parseFloat(tradeAmount) <= 0 ? '#4a5568' : '#fff', fontSize: 16, fontWeight: 800, cursor: !tradeAmount || parseFloat(tradeAmount) <= 0 ? 'not-allowed' : 'pointer', fontFamily: 'Inter, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  {placing ? (
                    <>
              ...
              <div style={{ background: '#0d1117', border: '1px solid #1e2530', borderRadius: 14, padding: '14px 16px' }}>
                <div style={{ fontSize: 11, color: '#2a3140', lineHeight: 1.7 }}>⚠ Trading involves significant risk. Leverage amplifies both profits and losses. Never trade more than you can afford to lose.</div>
              </div>
            </>
          )}
        </div>
